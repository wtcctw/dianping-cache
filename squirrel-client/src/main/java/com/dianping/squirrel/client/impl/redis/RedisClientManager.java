...
public class RedisClientManager implements Lifecycle {
	...
	private JedisCluster createClient(ThreadPoolExecutor executor) {
		JedisCluster client = new JedisCluster(getClusterNodes(clientConfig.getServerList()), connTimeout, readTimeout,
				maxRedirects, getPoolConfig(clientConfig), executor, clusterUpdateInterval, clientConfig.getPassword());
		return client;
	}
	...
	private ThreadPoolExecutor createExecutor() {
		return new ThreadPoolExecutor(asyncCoreSize, asyncMaxSize, 30L, TimeUnit.SECONDS,
				new ArrayBlockingQueue<Runnable>(asyncQueueSize), threadFactory,
