...
public class EhcacheClientTest {
	...
	public void testHotKey() throws Exception {
		...
		cacheClient.initialize(new EhcacheConfiguration());
		cacheClient.start();
		cacheClient.asyncSet(key, value, 100, true, null);
		Assert.assertEquals(value, cacheClient.get(key, Object.class, true, null, false));

		cacheClient.delete(key, true, null);
			...
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						if (cacheClient.get(key, Object.class, true, null, false) == null) {
	...
	public void testEternal() throws Exception {
		...
		cacheClient.initialize(new EhcacheConfiguration());
		cacheClient.start();
		cacheClient.asyncSet(key, value, 1, true, null);
		int i = 0;
		while (i++ < 20) {
			Assert.assertNotNull(cacheClient.get(key + "_bak", Object.class, null));
			Thread.sleep(1000);
		}
		cacheClient.stop();
	}
	...
	public void testHotKey3() throws Exception {
		...
		cacheClient.initialize(new EhcacheConfiguration());
		cacheClient.start();
		cacheClient.asyncSet(key, oldValue, 100, true, null);
		Assert.assertEquals(oldValue, cacheClient.get(key, Object.class, true, null, false));
		...
		for (int i = 0; i < threadCount; i++) {
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					try {
						start.await();
						Integer value = cacheClient.get(key, Object.class, true, null, false);
						if (value == null) {
							cacheClient.add(key, newValue, 100, true, null);
							Thread.sleep(3 * 1000);
						} else {
							if (value == oldValue) {
						...
						Thread.sleep(5 * 1000);
						value = cacheClient.get(key, Object.class, true, null, false);
						if (value == oldValue) {
							oldValueList.add(value);
						} else if (value == newValue) {
							newValueList.add(value);
