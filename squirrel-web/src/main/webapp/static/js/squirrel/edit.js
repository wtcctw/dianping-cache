...
module.controller('EditController', [
		...
		function($scope, $http,$timeout) {
			...
			$scope.initpage = function(){
				...
				$scope.wrapperParams();
				$scope.servers = [];
				$http.post(window.contextPath + '/config/configuration/1',$scope.configurationParams).success(function(response){
					$scope.mCacheKey = response.cacheKey;
					$scope.mClientClazz = response.clientClazz;
					$scope.mServers = response.servers;
					$scope.tempServers = response.servers;
					$scope.mTranscoderClazz = response.transcoderClazz;
