...
public class CacheKeyConfigurationServiceImpl implements CacheKeyConfigurationService {
	...
	public PageModel paginate(PageModel paginater, CacheKeyConfigSearchCondition searchCondition) {
		...
		p.setRecords(configurationDao.paginate((paginater.getPage()-1)*20, searchCondition));
		p.setRecordCount(configurationDao.paginate_COUNT(searchCondition));
		p.setPageSize(20);
		return p;
//		return configurationDao.paginate(paginater, searchCondition);
	...
	public void delete(String category) {
		...
		try {
			...
			// set version to -1 to indicate category removal
			configFound.setVersion(-1);
			cacheMessageProducer.sendMessageToTopic(translator.translate(configFound));
	...
	private void logCacheKeyConfigCreate(CacheKeyConfiguration config, boolean succeed) {
		operationLogService.create(succeed, "KeyConfigCreate", transferConfigDetail(config, null), true);
	}
	...
	private void logCacheKeyConfigUpdate(CacheKeyConfiguration oldConfig, CacheKeyConfiguration newConfig, boolean succeed) {
		...
		operationLogService.create(succeed, "KeyConfigUpdate", detail, true);
	}
