...
import com.dianping.cache.core.CacheFuture;
...
import com.dianping.cache.util.CacheKeyUtils;
...
public class DCacheClientImpl implements DCacheClient, Lifecycle, KeyAware, InitialConfiguration {
	...
	private DCacheClientConfiguration config;
	...
	public void init(CacheClientConfiguration config) {
		this.config = (DCacheClientConfiguration) config;
	}
	...
	private void initClient() {
		if (this.config == null || this.config.getLocator() == null || this.config.getTranscoder() == null
			...
			throw new InitializingException("invalid dcache config");
		}
		...
		try {
			...
			throw new CacheUnavailableException("error while initializing dcache", e);
		}
	}
			...
			ClientCallback<KVCacheResult> callback = new ClientCallback<KVCacheResult>() {
				...
				public void onCompleted(KVCacheResult result) {
					if (result.getCode() == DCacheConst.ET_SUCC) {
						...
						future.onFailure(new CacheException("dcache operation failed, error code:" + result.getCode()));
					}
				}
	...
	public boolean set(String key, Object value, int expiration, long timeout, boolean isHot, String category)
		...
		try {
			...
			throw new CacheException("dcache operation failed, error code:" + (result == null ? "" : result.getCode()));
		} catch (Exception e) {
			throw new CacheException(e);
		}
	}
	...
	public <T> Map<String, T> getBulk(Collection<String> keys, Class dataType, boolean isHot, Map<String, String> categories,
		...
		try {
			...
			throw new CacheException("dcache operation failed, error code:"
			...
			ClientCallback<KVCacheResult> dcacheCallback = new ClientCallback<KVCacheResult>() {
				...
				public void onCompleted(KVCacheResult result) {
					if (result.getCode() == DCacheConst.ET_SUCC) {
						...
						future.onFailure(new CacheException("DCache result code:" + result.getCode()));
					}
				}
    ...
    public <T> boolean batchSet(List<String> keys, List<T> values, int expiration, boolean isHot, String category)
        ...
        try {
            ...
            throw new CacheException("dcache batch set failed, error code: " + result.getCode());
        } catch(TimeoutException e) {
            throw e;
        } catch (Exception e) {
            throw new CacheException(e);
